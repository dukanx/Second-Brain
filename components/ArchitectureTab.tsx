...
const NODES: Record<string, NodeDetail> = {
  ...
  growth_tab: {
    ...
    description: '52-week activity heatmap, streak counter, spaced repetition review queue, task manager with subtasks, and weekly AI-generated summary.',
    tech: ['ISO week date math', 'Spaced repetition (7-day interval)', 'Claude weekly summary', 'JSONB subtasks in capture.text'],
    details: [
      '52-week GitHub-style heatmap + detailed monthly day view toggle',
      'Streak: consecutive days that have ≥1 capture created',
      'Review queue: Idea/Learning captures not reviewed in 7 days, max 5 shown at once',
      'Tasks: due_date, priority, subtasks stored as JSON array in capture.text field',
      'Weekly summary card: Mon–Sun captures → Claude analysis → upserted to summaries table',
    ],
  },
  digest_tab: {
    title: 'DigestTab.tsx',
    file: 'components/DigestTab.tsx',
    description: 'Daily digest of captures grouped by day with type distribution overview.',
    tech: ['Client-side date grouping', 'Type distribution'],
    details: [
      'Groups all captures by creation date (fully client-side, no API call)',
      'Shows type breakdown per day (Idea / Task / Link / etc.)',
      'Highlights starred captures within each day group',
    ],
  },
  api_capture: {
    title: '/api/capture',
...
const FLOWS: Record<FlowId, { title: string; description: string; steps: FlowStep[] }> = {
  ...
  chat: {
    ...
    steps: [
      ...
      { layer: 'db', component: 'Supabase', action: 'PATCH last_reviewed_at (only if "Mark Reviewed" clicked)', note: 'Triggers on explicit user action; removes capture from spaced rep queue' },
    ],
  },
  search: {
...
const FILE_TREE: FileNode[] = [
    ...
    name: 'app', type: 'dir', children: [
      {
        name: 'api', type: 'dir', children: [
          ...
          { name: 'digest/route.ts', type: 'file', detail: { description: 'Daily digest generation (groups captures by day)', exports: ['POST'], imports: ['lib/supabase/server'], usedBy: ['DigestTab'] } },
  ...
  },
  {
    name: 'components', type: 'dir', children: [
      { name: 'BrainClient.tsx', type: 'file', detail: { description: 'Hub: owns captures/projects state, 6 tabs, auth sign-out, visibility polling, SW notification routing, Cmd+K', exports: ['default', 'Capture (type)', 'Project (type)', 'TYPE_COLORS'], imports: ['lib/supabase/client', 'All tab components', 'SearchOverlay'], usedBy: ['app/brain/page.tsx'] } },
      ...
      { name: 'ChatTab.tsx', type: 'file', detail: { description: 'RAG chat: pinned context mode, conversation history, context capture display, mark-reviewed button', exports: ['default'], imports: ['BrainClient (Capture type)'], usedBy: ['BrainClient'] } },
      { name: 'SearchTab.tsx', type: 'file', detail: { description: 'Semantic search UI: query input → synthesis display → relevant capture cards → follow-up question buttons', exports: ['default'], imports: ['BrainClient (Capture type)'], usedBy: ['BrainClient'] } },
      { name: 'GraphTab.tsx', type: 'file', detail: { description: 'Force-directed knowledge graph: custom physics sim, canvas pan/zoom, batch relate, project/type filters', exports: ['default'], imports: ['BrainClient (Capture, Project types)'], usedBy: ['BrainClient'] } },
      { name: 'GrowthTab.tsx', type: 'file', detail: { description: '52-week heatmap, streak counter, spaced rep queue (max 5), task manager with subtasks, weekly Claude summary', exports: ['default'], imports: ['BrainClient (Capture type)'], usedBy: ['BrainClient'] } },
      { name: 'DigestTab.tsx', type: 'file', detail: { description: 'Daily digest: all captures grouped by day, type distribution, starred highlights', exports: ['default'], imports: ['BrainClient (Capture type)'], usedBy: ['BrainClient'] } },
      { name: 'SearchOverlay.tsx', type: 'file', detail: { description: 'Cmd+K quick-search modal: instant client-side filter across captures by text/title/type/project', exports: ['default'], imports: ['BrainClient (Capture, Project types)'], usedBy: ['BrainClient'] } },
...
const MAP_NODES: MapNode[] = [
  ...
  { id: 'mp_di',     col: 1, row: 6, label: 'DigestTab',       color: '#60a5fa', fileKey: 'DigestTab.tsx'    },
